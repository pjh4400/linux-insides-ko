...
[Interrupts and Interrupt Handling](https://junsoolee.gitbook.io/linux-insides-ko/summary/interrupts) 챕터의 일곱 번째 파트가 끝났고, 이 파트에서 외부 하드웨어 인터럽트에 대해 알아보기 시작했습니다. 외부 인터럽트에 대한 설명을 하고, irq 액션 목록, 인터럽트 핸들러에 대한 정보, 인터럽트 소유자, 처리되지 않은 인터럽트 수 등의 정보를 포함하는 `irq_desc`구조채의 초기 초기화를 보았습니다. 다음 부분에서는 외부 인터럽트를 계속 살펴봅시다.
