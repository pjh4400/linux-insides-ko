...
이러한 자원은 무한하지 않으며 각 프로세스마다이를 관리 할 도구가 있어야 합니다. 때로는 특정 자원에 대한 현재 제한을 알고 있거나 값을 변경하는 것이 유용합니다. 이 게시물에서는 프로세스 한계에 대한 정보를 얻고 그러한 한계를 늘리거나 줄일 수있는 도구를 고려할 것입니다.
...
마지막 함수는 약간 복잡해 보이고 `4` 인자를 취합니다. `리소스`인자 제외하고, 그것은:
...
이 모든 검사 후에 우리는 주어진 자원에 대한 한계를 업데이트하는 동안 [signal]() 핸들러 관련 것들이 파괴되지 않도록하기 위해 `tasklist`를 잠급니다:
